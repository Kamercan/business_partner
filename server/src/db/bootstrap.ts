...
/** Okunabilir ama tahmin edilemez geçici parola üretir. */
function randomPassword(): string {
  const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789';
  const bytes = crypto.randomBytes(20);
  const body = Array.from(bytes, (b) => alphabet[b % alphabet.length]).join('');
  return `${body.slice(0, 5)}-${body.slice(5, 10)}-${body.slice(10, 15)}!7`;
}

export function seedUsers(): { adminCreated: boolean; adminPassword: string | null } {
  ...
  const adminExists = db
    ...
    .get(config.seedAdminEmail) as { id: number } | undefined;

  let adminPassword: string | null = null;
  if (!adminExists) {
    // Parola verilmediyse: üretimde rastgele üret (loglara bir kez yazılır),
    // geliştirmede bilinen bir varsayılan kullan.
    adminPassword = config.seedAdminPassword || (config.isProd ? randomPassword() : 'Admin123!');
    insert.run(config.seedAdminEmail, hash(adminPassword), 'Sistem Yöneticisi', 'ADMIN', 'BT');
  }
...
/**
 * İlk açılış kurulumu. Şemayı uygular, referans verileri yükler ve gerekirse
...
export function bootstrapDatabase(): void {
  ...
  if (adminCreated) {
    ...
    console.log(`  E-posta : ${config.seedAdminEmail}`);
    if (adminPassword) {
      console.log(`  Parola  : ${adminPassword}`);
      console.log('  Bu parola yalnızca burada gösterilir — kaydedin ve');
      console.log('  giriş yaptıktan sonra Ayarlar ekranından değiştirin.');
    } else {
      console.log('  Parola  : SEED_ADMIN_PASSWORD ortam değişkeninde tanımlı.');
    }
    console.log(`${line}\n`);
  }
}
